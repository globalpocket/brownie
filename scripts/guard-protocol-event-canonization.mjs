...
export function validateRuntimeSemanticProtocolContract(contract, map, options = {}) {
  ...
  for (const terminalKind of ['TaskCompleted', 'TaskFailed', 'TaskCancelled']) {
    ...
    requireValue(
      terminalEntry?.payload_schema_classification === 'strict_typed',
      errors,
      `${contractPath} ${terminalKind} must be classified as strict_typed.`
    );
    requireValue(
      terminalEntry?.payload_schema_contract_status === 'closed' &&
        terminalEntry?.release_blocking_until_typed === false,
      errors,
      `${contractPath} ${terminalKind} strict typed payload schema must be closed and non-release-blocking.`
    );
    requireValue(
      terminalEntry?.payload_schema_descriptor?.includes('known_field_required:true') &&
        terminalEntry?.payload_schema_descriptor?.includes('additional_fields:false'),
      errors,
  ...
  const toolDescriptorRequirements = new Map([
    ['ToolPlanned', 'tool_planned_inventory_payload:true'],
    ['ToolPermissionChecked', 'tool_plan_decision_payload:true'],
    ['ToolPlanApproved', 'tool_plan_decision_payload:true'],
    ['ToolPlanDenied', 'tool_plan_decision_payload:true'],
    ['ToolIntentParsed', 'tool_intent_parsed_payload:true'],
    ['ToolIntentRejected', 'tool_intent_rejected_payload:true'],
    ['ToolIntentPermissionChecked', 'tool_intent_decision_payload:true'],
    ['ToolIntentApproved', 'tool_intent_decision_payload:true'],
    ['ToolIntentDenied', 'tool_intent_decision_payload:true'],
    ['ToolExecutionRequested', 'tool_execution_request_payload:true'],
    ['McpToolExecutionApproved', 'mcp_tool_execution_approval_payload:true'],
    ['ToolExecutionPermissionChecked', 'tool_execution_permission_payload:true'],
  ...
  for (const [toolKind, descriptorToken] of toolDescriptorRequirements) {
    ...
    requireValue(
      toolEntry?.payload_schema_classification === 'strict_typed',
      errors,
      `${contractPath} ${toolKind} must be classified as strict_typed.`
    );
    requireValue(
      toolEntry?.payload_schema_contract_status === 'closed' &&
        toolEntry?.release_blocking_until_typed === false,
      errors,
      `${contractPath} ${toolKind} strict typed payload schema must be closed and non-release-blocking.`
    );
    requireValue(
      toolEntry?.payload_schema_descriptor?.includes(descriptorToken) &&
        toolEntry?.payload_schema_descriptor?.includes('additional_fields:false'),
      errors,
  ...
  const codebaseDescriptorRequirements = new Map([
    ...
    ['CodebaseIndexQueryCompleted', 'codebase_index_query_payload:true'],
    ['CodebaseIndexSelectionReadCompleted', 'codebase_index_selection_read_payload:true'],
    ['CodebaseIndexPromptContextMaterialized', 'codebase_index_prompt_context_payload:true'],
    ['VerificationRecoveryContextReadMaterialized', 'verification_recovery_context_read_payload:true'],
  ]);
  for (const [eventKind, descriptorToken] of codebaseDescriptorRequirements) {
  ...
  const lifecycleDescriptorRequirements = new Map([
    ['AgentLoopStarted', 'agent_loop_started_payload:true'],
    ['AgentLoopCompleted', 'agent_loop_completed_payload:true'],
    ['TaskCompletionAccepted', 'task_completion_accepted_payload:true'],
    ['PromptBuilt', 'prompt_built_payload:true'],
    ['SecondPassPromptBuilt', 'prompt_built_payload:true'],
    ['PromptSensitiveScanCompleted', 'prompt_sensitive_scan_payload:true'],
    ['PromptSensitiveScanFailed', 'prompt_sensitive_scan_payload:true'],
    ['LlmRequestCreated', 'llm_request_created_payload:true'],
    ['SecondPassLlmRequestCreated', 'llm_request_created_payload:true'],
    ['LlmRequestFailed', 'llm_request_failed_payload:true'],
    ['SecondPassLlmRequestFailed', 'llm_request_failed_payload:true'],
    ['LlmResponseReceived', 'llm_response_received_payload:true'],
    ['SecondPassLlmResponseReceived', 'llm_response_received_payload:true'],
  ]);
  for (const [eventKind, descriptorToken] of lifecycleDescriptorRequirements) {
    const entry = fingerprintByKind.get(eventKind);
    requireValue(
      entry?.payload_schema_classification === 'strict_typed',
      errors,
    ...
    requireValue(
      entry?.payload_schema_contract_status === 'closed' &&
        entry?.release_blocking_until_typed === false,
      errors,
      `${contractPath} ${eventKind} strict typed lifecycle payload schema must be closed and non-release-blocking.`
    );
    requireValue(
  ...
  for (const terminalKind of ['TaskFailed', 'TaskCancelled']) {
    requireValue(
  ...
  for (const toolKind of toolDescriptorRequirements.keys()) {
    if (['ToolPlanApproved', 'ToolPlanDenied', 'ToolIntentApproved', 'ToolIntentDenied'].includes(toolKind)) {
      continue;
    }
    requireValue(
      schemaFixtures.some((fixture) => fixture?.ledger_event_kind === toolKind),
      errors,
      `${contractPath} must include ${toolKind} strict typed tool payload fixture.`
    );
  }
  for (const eventKind of codebaseDescriptorRequirements.keys()) {
    requireValue(
      schemaFixtures.some((fixture) => fixture?.ledger_event_kind === eventKind),
      errors,
      `${contractPath} must include ${eventKind} strict typed codebase/verification payload fixture.`
    );
  }
  for (const eventKind of lifecycleDescriptorRequirements.keys()) {
    requireValue(
      schemaFixtures.some((fixture) => fixture?.ledger_event_kind === eventKind),
      errors,
