...
fn objective_proposal_apply_params(
    ...
    if display_string(proposal, "operation")? != "replace_file" {
        debug_invalid_response_note("objective_proposal_apply_params", "operation is not replace_file");
        return Err(RuntimeClientError::InvalidResponse);
    }
    ...
    if display_string(proposal, "approval_status")? != "Approved" {
        debug_invalid_response_note("objective_proposal_apply_params", "proposal is not approved");
        return Err(RuntimeClientError::InvalidResponse);
    }
    if json_bool(
        proposal
        ...
        debug_invalid_response_note("objective_proposal_apply_params", "proposal content is truncated");
        return Err(RuntimeClientError::InvalidResponse);
    }
    if json_bool(
        proposal
        ...
        debug_invalid_response_note("objective_proposal_apply_params", "proposal diff is redacted");
        return Err(RuntimeClientError::InvalidResponse);
    }
    let replacement_content =
        required_workspace_write_content(proposal, "content_preview").map_err(|error| {
    ...
    let expected_target_sha256 = required_display_string(snapshot, "file_sha256").map_err(|error| {
        debug_invalid_response_note("objective_proposal_apply_params", "snapshot file_sha256 missing");
        error
    })?;
    ...
    let run_id = required_display_string(authorization, "run_id").map_err(|error| {
        debug_invalid_response_note("objective_proposal_apply_params", "authorization run_id missing");
        error
    })?;
    Ok(Some(json!({
        "run_id": run_id,
