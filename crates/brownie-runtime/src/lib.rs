...
use brownie_protocol::{
    ...
    WorkspacePatchProposalAuditEntry, WorkspacePatchProposalAuditTrailSummary,
...
const MAX_PROPOSAL_AUDIT_TRAIL_EVENTS: usize = 50;
...
fn inspect_proposal_audit_trail(
    ...
    (
        WorkspacePatchProposalSummary,
        WorkspacePatchProposalAuditTrailSummary,
    ),
    String,
> {
    let proposal = inspect_proposal(store, run_id, proposal_id)?;
    let events = read_existing_run_events(store, run_id)?;
    let entries: Vec<WorkspacePatchProposalAuditEntry> = events
        .iter()
        .filter_map(|event| proposal_audit_entry_from_event(event, proposal_id))
        .collect();
    ...
    let events = entries
        .iter()
        .rev()
        .take(MAX_PROPOSAL_AUDIT_TRAIL_EVENTS)
        .cloned()
        .collect::<Vec<_>>()
        .into_iter()
        .rev()
        .collect();
    let audit_trail = WorkspacePatchProposalAuditTrailSummary {
        proposal_id: proposal_id.to_string(),
        event_count,
        latest_event,
        events,
...
fn proposal_audit_entry_from_event(
    event: &LedgerEvent,
    proposal_id: &str,
) -> Option<WorkspacePatchProposalAuditEntry> {
    let payload = sanitize_ledger_payload(event.payload.clone())?;
    if payload.get("proposal_id").and_then(Value::as_str) != Some(proposal_id) {
        return None;
    }
    let (audit_event, summary, metadata) = match event.kind {
        ...
        LedgerEventKind::WorkspacePatchApproved => (
            "proposal_approved",
            "Proposal approved; no apply performed.",
            audit_metadata(
                &payload,
                &[
                    "approval_status",
        ...
        LedgerEventKind::WorkspacePatchRejected => (
            "proposal_rejected",
            "Proposal rejected; no apply performed.",
            audit_metadata(
                &payload,
                &[
                    "approval_status",
        ...
        LedgerEventKind::WorkspacePatchPreflightSnapshotCreated => (
            ...
            "Preflight snapshot captured without applying the proposal.",
            audit_metadata(
                &payload,
                &[
                    "snapshot_id",
        ...
        LedgerEventKind::WorkspacePatchApplyPlanCreated => (
            "apply_plan_created",
            "Apply plan created for review only.",
            audit_metadata(
                &payload,
        ...
        LedgerEventKind::WorkspacePatchReadinessReportCreated => (
            "readiness_checked",
            "Readiness checked without applying the proposal.",
            audit_metadata(
                &payload,
                &[
                    "report_id",
        ...
        LedgerEventKind::WorkspacePatchApplyCapabilityChecked => (
            ...
            "Apply capability inspected; runtime apply remains disabled.",
            audit_metadata(
                &payload,
                &[
                    "capability_id",
    ...
    Some(WorkspacePatchProposalAuditEntry {
        proposal_id: proposal_id.to_string(),
...
fn audit_metadata(payload: &Value, keys: &[&str]) -> Value {
...
mod tests {
    ...
    fn implementer_workspace_write_creates_proposal_without_modifying_file() {
        ...
        assert!(audit_events.iter().any(|event| {
            event["audit_event"] == "readiness_checked"
                && event["metadata"]["readiness_status"] == "Ready"
        }));
        assert!(audit_events.iter().any(|event| {
            event["audit_event"] == "apply_capability_checked"
                && event["metadata"]["apply_supported"] == false
                && event["metadata"]["apply_enabled"] == false
        }));
        assert!(audit_events.iter().all(|event| {
            event["proposal_id"] == proposal_id
                && event["metadata"].get("content_preview").is_none()
                && event["metadata"].get("diff_preview").is_none()
        }));
