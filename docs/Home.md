...
| **記憶エンジン (Vector DB Server)** | ChromaDB (Server Mode) または Qdrant。| 記憶の永続化と類似性検索による過去事例の呼び出し。 |
| **解析エンジン (Code Index)** | DuckDB | 高速な SQL クエリによるシンボル解析・フロー追跡。 |
...
### 2.4 Database Architecture (役割の分離)

BROWNIE は、データの性質に応じて 2 種類のデータベースを使い分けている。

| DB | ライブラリ | 主な役割 | 特徴 |
| --- | --- | --- | --- |
| **SQLite** | `aiosqlite` | タスクの状態管理、実行履歴、コンテキストの永続化。 | ACID 特性を重視し、システムの再起動後もタスクを継続するための信頼性を提供。 |
| **DuckDB** | `duckdb` | ソースコードのシンボルマップ、呼び出し関係のインデックス。 | 列指向 DB の高速性を活かし、大規模コードベースに対する複雑な静的解析をミリ秒単位で処理。 |
...
* **StateManager (状態管理):** 起動時に整合性チェックを実行。OSクラッシュ等による **Stale Lock（`.shm` / `.wal` 不正残存）を自動検知し復旧**。
  * SQLite仕様: WALモード。
  * 役割: タスクキューの管理、エージェントの思考履歴の永続化。
* **CodeAnalyzer (コード解析):** DuckDB を用いた高速なシンボル検索とフロー解析を提供。
  * 役割: `get_repository_flow` などのツールに背面で情報を提供し、AST ベースの深いコンテキスト理解を実現。
...
### 7.1. 対応処理 (Git Flow 準拠)

一般実装タスクにおいて、Brownie は以下の手順で作業を完遂しなければならない：

1. **Issueの内容とIDを取得**: 対象の Issue タイトル、本文、および Issue 番号を特定する。
2. **トピックブランチの作成**: 「`issue-[ID]`」形式のブランチを `main` から作成し、切り替える。
3. **自律実装**: 作成したトピックブランチ内で、エージェントによる調査・実装・検証ループを実行する。
4. **プッシュ**: 変更内容をトピックブランチへコミットし、リモートへプッシュする。
5. **PR作成**: GitHub 上で `main` ブランチに対するプルリクエストを作成し、人間へレビューを依頼する。
