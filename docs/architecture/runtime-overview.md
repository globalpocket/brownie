...
M7.2 extends that fixed-verifier model with `verification.cargo_check`. The tool requires `ExecuteProcess`, accepts only `{}` or `{ "check_id": "cargo_check" }`, runs exactly `cargo check --workspace --all-targets --locked --offline`, requires workspace `Cargo.toml` and `Cargo.lock`, rejects workspaces containing `build.rs` in this phase, uses a runtime-owned isolated Cargo target directory outside the workspace, disables Cargo network access, removes the isolated target directory after execution, and records only bounded verifier metadata including `target_dir_isolated`, `network_disabled`, and `cleanup_succeeded`.
