...
Audited main: `5320e89d61db940c4168b0e339cfe7ad1a7db924`
...
verified legacy v1 through v11 payload-envelope read compatibility. The other
11 current headless continuation/run/journey ledger payload families remain
`versioned_open` partial required-before-release states rather than full typed
schema coverage.
...
| `runtime-release-guard-ci` | P0 | CI Release Gate | partial | Runtime | required before release | RRP-3.1 adds the process-loss E2E to the existing VSIX check script that CI invokes; RRP-4.1 adds durable schema migration behavior tests plus a guard that detects deletion/drift of the migration evidence, but direct `.github` workflow wiring remains blocked by missing OAuth `workflow` scope; RRP-5/RRP-5.1/RRP-5.2/RRP-5.3/RRP-5.4/RRP-5.5/RRP-5.6/RRP-5.7/RRP-5.8/RRP-5.9/RRP-5.10/RRP-5.11/RRP-5.12/RRP-5.13/RRP-5.14/RRP-5.15/RRP-5.16, RRP-6, RRP-7, RRP-7.1, and RRP-7.2 add full protocol method semantic contracts, recursive nested wire schemas, ledger payload schema/instance fingerprint split evidence, explicit per-event payload classification debt, terminal task strict typed payload schemas, permission decision strict typed payload schemas, selected tool strict typed payload schemas, MCP approval/tool terminal strict typed payload schemas, task/LLM lifecycle strict typed payload schemas, task admission/mode-resolution strict typed payload schemas, tool planning/intent parsing strict typed payload schemas, subtask orchestration/dispatch strict typed payload schemas, workspace patch strict typed/payload_absent schemas, module-decomposition, platform/deadline/durability, persisted deadline, failure-injection, terminal-race, Windows durability, Windows stale-lock recovery, and MCP crash-window guards to that same CI-invoked path. Full release-gate hardening remains open for frozen install, fmt, audit/SBOM/secret/dependency, and complete release policy coverage. | Close direct workflow wiring and remaining full CI release-gate hardening only after workflow-scope credentials or an owner-approved equivalent CI-hardening path are available. |
...
| `protocol-event-canonization` | P1 | Protocol/Event canonization | partial | Runtime | required before release | RRP-5 adds a canonical ownership/drift map; RRP-5.1 adds a Rust-generated semantic protocol contract artifact; RRP-5.2 expands it to all 51 explicit Runtime methods and all public Runtime params unknown-field evidence; RRP-5.3 adds recursive `schemars` JSON Schema `type_schemas` with nested `$defs`, method schema refs/fingerprints and VSIX nested-schema coverage; RRP-5.4 separates fixed ledger payload schema fingerprints from diagnostic instance-shape fingerprints, validates v2 envelopes on append/read, and preserves verified legacy v1 envelope compatibility; RRP-5.5 records one payload schema classification per `LedgerEventKind` and blocks release while `typed_known_fields_open` or `versioned_open` classifications remain; RRP-5.6 requires known terminal evidence for `TaskCompleted`; RRP-5.7 moves `TaskCompleted`, `TaskFailed`, and `TaskCancelled` to strict typed terminal task payload schemas; RRP-5.8 moves `PermissionChecked` and `PermissionDenied` to strict typed permission decision payload schemas; RRP-5.9 moves selected tool planning, intent, execution request, execution permission, and execution denial payload schemas to strict typed contracts and reduces release-blocking open payload families to 63; RRP-5.10 moves MCP approval state plus tool completion/failure terminal payload schemas to strict typed contracts and reduces open payload families to 60; RRP-5.11 moves Codebase Index and verification recovery context payload schemas to strict typed contracts and reduces release-blocking open payload families to 54; RRP-5.12 moves task/LLM lifecycle payload schemas to strict typed contracts and reduces release-blocking open payload families to 41; RRP-5.13 moves task admission, running-deadline/admission, mode-resolution, and external ModePack provenance-denial payload schemas to strict typed contracts and reduces release-blocking open payload families to 36. RRP-5.14 moves ToolPlanned, ToolIntentParsed, and ToolIntentRejected to strict typed tool planning/intent parsing contracts and reduces release-blocking open payload families to 33; RRP-5.15 moves subtask orchestration/dispatch payload schemas to strict typed contracts and reduces release-blocking open payload families to 21; RRP-5.16 moves workspace patch payload schemas to strict typed contracts, classifies the unused approval-request event as payload_absent, and reduces release-blocking open payload families to 11. | Assign strict typed, payload-absent, or legacy-compatibility schemas to every remaining 11 headless continuation/run/journey payload `LedgerEventKind` before treating this blocker as closed. |
