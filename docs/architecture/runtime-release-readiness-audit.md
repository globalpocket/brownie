...
RRP-5/RRP-5.1/RRP-5.2 closes the protocol/event canonization blocker. The canonical
ownership map at
...
`LedgerEventKind` payload family. VSIX tests consume the same artifact to prove
canonical method coverage, client projection, validator semantics, and payload
envelope fixture evidence. The existing CI-invoked VSIX check path runs this
guard.
...
| `runtime-release-guard-ci` | P0 | CI Release Gate | partial | Runtime | required before release | RRP-3.1 adds the process-loss E2E to the existing VSIX check script that CI invokes; RRP-4.1 adds durable schema migration behavior tests plus a guard that detects deletion/drift of the migration evidence, but direct `.github` workflow wiring remains blocked by missing OAuth `workflow` scope; RRP-5/RRP-5.1/RRP-5.2, RRP-6, RRP-7, and RRP-7.1 add full protocol/event semantic contracts, module-decomposition, platform/deadline/durability, absolute deadline, failure-injection, and terminal-race guards to that same CI-invoked path. Full release-gate hardening remains open for frozen install, fmt, audit/SBOM/secret/dependency, and complete release policy coverage. | Close direct workflow wiring and remaining full CI release-gate hardening only after workflow-scope credentials or an owner-approved equivalent CI-hardening path are available. |
