...
Audited main: `c2acdc327755ef02307b8afd953b7edafde8c23a`
...
payload-envelope read compatibility. RRP-5.14 moves `ToolPlanned`,
`ToolIntentParsed`, and `ToolIntentRejected` to strict typed tool planning
and intent parsing payload schemas with verified legacy v1 through v9
payload-envelope read compatibility. The other 33 current ledger
payload families remain `versioned_open` partial
required-before-release states rather than full typed schema coverage.
