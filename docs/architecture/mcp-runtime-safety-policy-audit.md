...
Audited main: `84cafe8d1bc37a352c12a9cfd0467a5ba19a9cd2`

| ID | Priority | Area | Status | Classification | Evidence summary | Next action |
| --- | --- | --- | --- | --- | --- | --- |
| `mcp-result-error-semantics` | P0 | MCP `isError` and execution status semantics | implemented sufficient | closed | MCP `tools/call` now separates `ProtocolSucceeded`, `ToolSucceeded`, `ToolReturnedError`, `ProtocolFailed`, `TimedOut`, and malformed-result `Failed` outcomes while preserving the external `ToolExecuteStatus` enum. `isError=true` emits `ToolExecutionFailed`, carries only bounded untrusted context, is not completion evidence, and is not a completed-success replay entry. | Use as the baseline for later MCP safety phases. |
