...
`proposal.reviewQueueDiagnostics` accepts `{ "run_id": string }` and returns `{ "review_queue_diagnostics": WorkspacePatchReviewQueueDiagnosticsSummary }`. Diagnostics reconstruct the existing review queue, verify count and status consistency, ensure queue items do not authorize apply, require compact non-absolute evidence fields, and return deduplicated required next actions. Diagnostics are `Blocked` when consistency checks fail or any queue item is blocked, `NeedsAction` when the queue is consistent but needs action, and `Complete` when the queue is consistent and complete. Diagnostics append no ledger event, always return `apply_authorized = false`, and must preserve the same no-write, no-apply, no raw-content boundary as `proposal.reviewQueue`.
