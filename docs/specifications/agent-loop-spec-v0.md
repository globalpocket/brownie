...
M62.1 extends that same derivation and decision path with runtime-selected Product DoD gap evidence. If a project completion policy asserts `product_completion_claim = false`, it must provide a bounded `product_dod_remaining_gaps` array. The runtime validates each gap's id, capability, transition, status, required flag, priority, and next action, rejects malformed or duplicate gap evidence before appending `HeadlessRunProductEvidenceMatrixDerived`, selects one open required gap deterministically by highest numeric priority then stable gap id, and includes its bounded metadata plus a selection fingerprint in the matrix fingerprint and payload. A subsequent `product_completion_decision` referencing that matrix may omit `remaining_capability`; the runtime derives it from the selected gap and includes the same selected-gap metadata in the bounded decision result. If a caller requests `product_complete` while the derived matrix has an open required Product DoD gap, the runtime denies the request before appending `HeadlessRunProductCompletionDecisionRecorded`. This adds no new RPC, report, digest, history, readiness, inspection, preview, verdict, automatic provider/tool execution, shell, git, network, service, or workspace mutation, and does not expose raw policy, evidence artifact, prompt, provider, file, output, command, environment, path, or secret content.

M63.1 closes the selected Product DoD gap loop through existing `headless.run.drive`. A caller may provide one explicitly authorized `selected_product_gap_closure` envelope on a later accepted selected-gap-derived objective completion boundary. The runtime revalidates the source product decision fingerprint, source product evidence fingerprint, selected remaining gap fingerprint, derived product objective fingerprint, accepted completion fingerprint, terminal completion fingerprint, and completion closure fingerprint before recording one bounded `HeadlessRunSelectedProductGapClosureRecorded` event. Product evidence matrix derivation then consumes matching closure evidence so that exact selected gap is treated as closed and is not selected again as open work; if no other open required gaps remain, the matrix can support a `product_complete` decision on that later boundary. Replay remains duplicate-free, stale or conflicting closure requests fail before mutation, no JSON-RPC method is added, and raw prompt, provider, file, output, command, environment, path, secret, policy text, and raw ledger payload content remain excluded.

M63.2 extends that same matrix derivation path for multi-gap completion. When more than one open required Product DoD gap is closed by matching runtime-recorded selected-gap closure evidence, the matrix now carries a bounded `selected_gap_closure_evidence_set` plus a `selected_gap_closure_set_fingerprint`; the matrix fingerprint commits the full consumed set rather than only the legacy single `selected_gap_closure_evidence` compatibility field. If any required gap remains unclosed, the matrix still selects the highest-priority remaining gap and denies effective product completion. No new JSON-RPC method, report, inspection, readiness wrapper, or caller-authored gap closure status is introduced.
