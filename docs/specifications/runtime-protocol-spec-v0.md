...
Before queueing, the runtime resolves requested `mode_id` values against the workspace mode policy set. Unknown modes use the existing `ToolIntentDenied` event path and do not create queued subtask evidence or child records. Valid requested inputs are persisted only as sanitized summary fields: `requested_goal_preview` and `requested_mode_id`.
