...
`WorkspacePatchReviewQueueDiagnosticsSummary` is summary-only and contains `run_id`, `diagnostics_status`, `diagnostics_reason`, `queue_status`, `proposal_count`, `complete_count`, `needs_action_count`, `blocked_count`, `check_count`, `failed_checks`, `blocked_checks`, `checks`, `required_next_actions`, `apply_authorized`, and `generated_at`. Each diagnostic check contains `name`, `status`, and `reason`; check statuses are `Pass`, `Fail`, and `Blocked`.

Diagnostics verify that queue counts match item statuses, queue status matches item statuses, every queue item has `apply_authorized = false`, queue items expose only compact review evidence fields, required next actions are deduplicated summary strings, and diagnostics remain inspection-only. `diagnostics_status` is `Blocked` when consistency checks fail, apply authorization evidence is present, or any queue item is blocked; `NeedsAction` when the queue is consistent but needs action; and `Complete` when the queue is consistent and complete. `apply_authorized` is always `false`.

`proposal.reviewQueueDiagnostics` is reconstructed from the existing `proposal.reviewQueue` evidence and appends no ledger event. It must not apply patches, write workspace files, run shell or git commands, use network access, expose canonical absolute paths, or return/store raw file content, raw diffs, raw input JSON, `content`, `raw_content`, `full_content`, `patch`, `diff`, `raw_input`, `canonical_path`, `absolute_path`, or `file_content`.
