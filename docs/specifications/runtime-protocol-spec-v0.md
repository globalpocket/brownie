...
`llm.status` returns `provider`, `enabled`, `model`, `base_url`, `reason`,
`strict`, `will_fallback_to_fake`, `llm_provider_access_allowed`, and the
compatibility alias `task_run_network_allowed`. `will_fallback_to_fake` is true
only when OpenAI-compatible was requested, required configuration is missing,
and `BROWNIE_LLM_STRICT` is not true. No API key or Authorization/Bearer value
is returned.
...
`BROWNIE_LLM_ALLOW_PROVIDER_ACCESS=true` is the preferred guard required before strict enabled OpenAI-compatible `task.run` may make provider LLM calls. The legacy `BROWNIE_LLM_ALLOW_TASK_RUN_NETWORK=true` guard is accepted only when the preferred guard is unset. If both guards are set and disagree, Runtime fails closed before any provider request and diagnostics report `TASK_RUN_NETWORK_GUARD_CONFLICT`. The default is false. `llm.status` and `runtime.config.get` expose `llm_provider_access_allowed`; they also keep `task_run_network_allowed` as a compatibility alias with the same value during migration. `runtime.diagnostics.get` reports `TASK_RUN_NETWORK_ALLOWED`, `TASK_RUN_NETWORK_NOT_ALLOWED`, or `TASK_RUN_NETWORK_GUARD_CONFLICT` for strict enabled OpenAI-compatible profiles. Missing guard is a warning in diagnostics and a pre-network `task.run` error. Non-strict OpenAI-compatible `task.run` falls back to Fake. See `docs/specifications/real-provider-task-run-smoke-spec-v0.md`.
...
`proposal.apply` accepts `{ "run_id": string, "proposal_id": string, "expected_target_sha256": string, "replacement_content": string, "authorize": true }` and returns `{ "proposal": WorkspacePatchProposalSummary, "apply_result": WorkspacePatchApplyResultSummary }`. Empty IDs, missing expected hashes, missing content, unknown runs, and unknown proposals return JSON-RPC `-32602`. Apply revalidates `WriteWorkspace` against the proposal's concrete path and the source run's durable compiled `workspace_write_scopes` before mutation; out-of-scope apply records a denied result and does not consume the one-time authorization.
...
`FetchSelectedModePackCandidateExplicitly` route boundary by accepting a bounded
`journey_route_resume` object. The resume request contains
`authorize_journey_route_resume=true`, a bounded `journey_id`, the expected
journey fingerprint, expected route kind, expected source registry-selection
...
`modepack_selected_candidate_fetch_target`; Rust derives that target from the
persisted registry-selection checkpoint.

Before route side effects, the runtime validates the journey checkpoint, current
session checkpoint, route kind, expected source checkpoint fingerprint, source
selection identity, replay identity, unit route budget, and absence of mixed
journey admission, explicit Mode Pack targets, context budgets, or completion
finalization authorization. Stale, malformed, unsupported, or missing-checkpoint
requests fail closed.

On success, the runtime delegates to existing `headless.run.advance` /
`headless.continue_once` selected-candidate fetch authority, persists the normal
fetch and drive checkpoints, appends bounded `HeadlessJourneyRouteResumed`
ledger evidence to the journey task, and returns
`HeadlessRunDriveResult.journey_route_resume`. That metadata contains only
journey/task/run/session/drive handles, route kind, source continuation and
...
identity returns the committed metadata without duplicating selected-candidate
fetch, drive, or journey-route resume evidence.

## M50.3 headless provenance route resume

`headless.run.drive` can resume one already-admitted journey across the
`VerifySelectedModePackCandidateProvenanceExplicitly` route boundary through
the same bounded `journey_route_resume` object. The resume request contains
`authorize_journey_route_resume=true`, a bounded `journey_id`, the expected
journey fingerprint, expected route kind, expected selected-candidate fetch
checkpoint fingerprint, the expected current session sequence, and an explicit
drive id. The successful request must not include
`modepack_selected_candidate_provenance_verification_target`; Rust derives that
target from the persisted selected-candidate fetch checkpoint.

Before route side effects, the runtime validates the journey checkpoint, current
session checkpoint, route kind, expected source checkpoint fingerprint, replay
identity, unit route budget, and absence of mixed journey admission, explicit
Mode Pack targets, context budgets, or completion finalization authorization.
Stale, malformed, unsupported, or missing-checkpoint requests fail closed.

On success, the runtime delegates to existing `headless.run.advance` /
`headless.continue_once` provenance verification authority, persists the normal
provenance and drive checkpoints, appends bounded `HeadlessJourneyRouteResumed`
ledger evidence to the journey task, and returns
`HeadlessRunDriveResult.journey_route_resume` with
`derived_target_class:
"modepack_selected_candidate_provenance_verification_target"`. The metadata
...
deterministic resume fingerprint. Replaying the same drive id and resume
identity returns the committed metadata without duplicating provenance
verification, advance, drive, or journey-route resume evidence. The response
