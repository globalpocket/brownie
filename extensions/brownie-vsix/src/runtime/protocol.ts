...
export interface WorkspacePatchReviewQueueDiagnosticCheckSummary {
  name: string;
...
export interface WorkspacePatchReviewQueueDiagnosticsSummary {
  ...
  failed_checks: string[];
  blocked_checks: string[];
  checks: WorkspacePatchReviewQueueDiagnosticCheckSummary[];
  required_next_actions: string[];
  apply_authorized: false;
  generated_at: string;
}
...
export function isWorkspacePatchReviewQueueDiagnosticCheckSummary(value: unknown): value is WorkspacePatchReviewQueueDiagnosticCheckSummary {
...
export function isWorkspacePatchReviewQueueDiagnosticsSummary(value: unknown): value is WorkspacePatchReviewQueueDiagnosticsSummary {
  return isRecord(value) && typeof value.run_id === 'string' && (value.diagnostics_status === 'Complete' || value.diagnostics_status === 'NeedsAction' || value.diagnostics_status === 'Blocked') && typeof value.diagnostics_reason === 'string' && (value.queue_status === 'Complete' || value.queue_status === 'NeedsAction' || value.queue_status === 'Blocked') && isNonNegativeInteger(value.proposal_count) && isNonNegativeInteger(value.complete_count) && isNonNegativeInteger(value.needs_action_count) && isNonNegativeInteger(value.blocked_count) && isNonNegativeInteger(value.check_count) && Array.isArray(value.failed_checks) && value.failed_checks.every((check) => typeof check === 'string') && Array.isArray(value.blocked_checks) && value.blocked_checks.every((check) => typeof check === 'string') && Array.isArray(value.checks) && value.checks.every(isWorkspacePatchReviewQueueDiagnosticCheckSummary) && Array.isArray(value.required_next_actions) && value.required_next_actions.every((action) => typeof action === 'string') && value.apply_authorized === false && typeof value.generated_at === 'string' && hasNoForbiddenRawFields(value);
}
