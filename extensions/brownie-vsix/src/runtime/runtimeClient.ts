...
import type { ModePackActivateResult, ModePackApproveCandidateResult, ModePackFetchCandidateResult, ModePackReplaceActiveResult, ModePackRevokeSignerResult, ModePackRollbackActiveResult, ModePackTrustSignerResult, ModePackVerifyCandidateProvenanceResult } from './protocol';
...
import { isModePackActivateResult, isModePackApproveCandidateResult, isModePackFetchCandidateResult, isModePackReplaceActiveResult, isModePackRevokeSignerResult, isModePackRollbackActiveResult, isModePackTrustSignerResult, isModePackVerifyCandidateProvenanceResult } from './protocol';
...
export class RuntimeClient {
  ...
  async revokeModePackSigner(authorizeRevocation: boolean, signerFingerprint: string): Promise<ModePackRevokeSignerResult> {
    const result = await this.call<ModePackRevokeSignerResult>('modepack.revokeSigner', {
      authorize_revocation: authorizeRevocation,
      signer_fingerprint: signerFingerprint,
    });

    if (!isModePackRevokeSignerResult(result)) {
      throw new RuntimeProtocolError('modepack.revokeSigner returned an invalid result');
    }
