...
describe('protocol validation', () => {
  ...
  it('accepts valid llm.status results and rejects missing required fields', () => {
    expect(isLlmStatusResult({ provider: 'Fake', enabled: true, model: 'brownie-fake-llm', base_url: null, reason: null, strict: false, will_fallback_to_fake: false, llm_provider_access_allowed: false, task_run_network_allowed: false, config_source: 'Default', active_profile: null, budget: { max_prompt_chars: 120000, max_messages: 64, request_timeout_ms: 30000, response_preview_chars: 2000 }, sensitive_guard: 'warn' })).toBe(true);
    expect(isLlmStatusResult({ provider: 'Unknown', enabled: false, model: '', base_url: null, reason: 'unknown provider: mystery', strict: true, will_fallback_to_fake: false, llm_provider_access_allowed: false, task_run_network_allowed: false, config_source: 'Env', active_profile: null, budget: { max_prompt_chars: 120000, max_messages: 64, request_timeout_ms: 30000, response_preview_chars: 2000 }, sensitive_guard: 'warn' })).toBe(true);
    expect(isLlmStatusResult({ provider: 'Fake', enabled: true, model: 'brownie-fake-llm', will_fallback_to_fake: false, llm_provider_access_allowed: false, task_run_network_allowed: false, config_source: 'Default', budget: { max_prompt_chars: 120000, max_messages: 64, request_timeout_ms: 30000, response_preview_chars: 2000 }, sensitive_guard: 'warn' })).toBe(false);
    expect(isLlmStatusResult({ provider: 'Fake', enabled: true })).toBe(false);
    expect(isLlmStatusResult({ provider: 'Fake', enabled: true, model: 'brownie-fake-llm', base_url: null, reason: null, strict: false, will_fallback_to_fake: false, llm_provider_access_allowed: false, task_run_network_allowed: false, active_profile: null, budget: { max_prompt_chars: 120000, max_messages: 64, request_timeout_ms: 30000, response_preview_chars: 2000 }, sensitive_guard: 'warn' })).toBe(false);
    expect(isLlmStatusResult({ provider: 'Fake', enabled: true, model: 'brownie-fake-llm', base_url: null, reason: null, strict: false, will_fallback_to_fake: false, llm_provider_access_allowed: false, task_run_network_allowed: false, config_source: 'Default', api_key: 'secret' })).toBe(false);
    expect(isLlmStatusResult({ provider: 'Fake', enabled: true, model: 'brownie-fake-llm', base_url: null, reason: null, strict: false, will_fallback_to_fake: false, llm_provider_access_allowed: false, task_run_network_allowed: false, config_source: 'Default', active_profile: null })).toBe(false);
  ...
  it('accepts bounded modepack.replaceActive results and rejects raw fields', () => {
    ...
    const updateAdmission = {
      ...
      source_url_host: approvedCandidate.source_url_host,
      source_url_fingerprint: approvedCandidate.source_url_fingerprint,
      dns_binding: modePackDnsBinding,
  ...
  it('accepts bounded modepack.approveCandidate results and rejects raw fields', () => {
    ...
    const result = {
      ...
      replayed: false,
      approval,
      next_action: 'replace_active_with_approved_modepack_candidate',
    };

    expect(isModePackApproveCandidateResult(result)).toBe(true);
    expect(isModePackApproveCandidateResult({ ...result, approval: { ...approval, content_sha256: 'bad' } })).toBe(false);
  ...
  it('accepts bounded modepack.rollbackActive results and rejects raw fields', () => {
    const current = {
      activation_id: 'modepack_activation_current',
      activation_fingerprint: `sha256:${'a'.repeat(64)}`,
      modepack_name: 'local-agentmodes',
      schema_version: 1,
      source_kind: 'workspace_modepack',
    ...
    const restored = {
      ...current,
      activation_id: 'modepack_activation_restored',
      activation_fingerprint: `sha256:${'c'.repeat(64)}`,
      mode_ids: ['reviewer-lite'],
      compiled_policy_fingerprint: `sha256:${'d'.repeat(64)}`,
      activation_event_id: 'event_2',
    ...
    const result = {
      ...
      rollback_event_id: 'event_3',
    };

    expect(isModePackRollbackActiveResult(result)).toBe(true);
    expect(isModePackRollbackActiveResult({ ...result, restored_snapshot: { ...restored, activation_fingerprint: 'bad' } })).toBe(false);
    expect(isModePackRollbackActiveResult({ ...result, raw_modepack_json: '{}' })).toBe(false);
    expect(isModePackRollbackActiveResult({ ...result, raw_ledger_payload: {} })).toBe(false);
  });
  ...
  it('accepts run inspection summaries and sanitized event payloads', () => {
    ...
    const objectiveCompletionAcceptanceTarget = {
      authorize_objective_completion_acceptance: true,
      objective_apply_verification_continuation_id: 'm56.apply.verify',
      expected_objective_apply_verification_decision_id: `headless_decision_${'b'.repeat(32)}`,
      journey_id: objectiveProposalAuthorizationPreflightTarget.journey_id,
      session_id: objectiveProposalAuthorizationPreflightTarget.session_id,
      source_drive_id: objectiveProposalAuthorizationPreflightTarget.source_drive_id,
      expected_task_id: objectiveProposalAuthorizationPreflightTarget.expected_task_id,
      ...
      expected_apply_fingerprint: `sha256:${'9'.repeat(64)}`,
      expected_post_write_sha256: `sha256:${'a'.repeat(64)}`,
      expected_current_target_sha256: `sha256:${'a'.repeat(64)}`,
      expected_verification_status: 'verified',
      expected_verification_route_kind: 'accept_objective_completion_explicitly',
      expected_verification_fingerprint: `sha256:${'b'.repeat(64)}`,
    };
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
      objective_completion_acceptance_target: objectiveCompletionAcceptanceTarget,
    })).toBe(true);
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
      objective_completion_acceptance_target: {
        ...objectiveCompletionAcceptanceTarget,
        expected_verification_status: 'mismatch',
      },
    })).toBe(false);
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
      objective_completion_acceptance_target: {
        ...objectiveCompletionAcceptanceTarget,
        expected_verification_fingerprint: 'not-a-fingerprint',
      },
    })).toBe(false);
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
    ...
    const modePackSelectedCandidateProvenanceVerificationTarget = {
      authorize_selected_candidate_provenance_verification: true,
      fetch_continuation_id: 'continue.once:modepack.fetch',
      expected_fetch_decision_id: `headless_decision_${'a'.repeat(32)}`,
      ...
      provenance_statement_json: '{"content_sha256":"bounded"}',
      provenance_signature_base64: 'a'.repeat(88),
      provenance_public_key_base64: 'b'.repeat(44),
    };
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
    ...
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
      modepack_selected_candidate_provenance_verification_target: { ...modePackSelectedCandidateProvenanceVerificationTarget, authorize_selected_candidate_provenance_verification: false },
    })).toBe(false);
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
      modepack_selected_candidate_provenance_verification_target: { ...modePackSelectedCandidateProvenanceVerificationTarget, expected_fetch_decision_id: 'fetch_decision_bad' },
    })).toBe(false);
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
      modepack_selected_candidate_provenance_verification_target: { ...modePackSelectedCandidateProvenanceVerificationTarget, expected_signer_fingerprint: 'not-a-fingerprint' },
    })).toBe(false);
    expect(isHeadlessContinueOnceParams({
      ...headlessParams,
    ...
    const selectedCandidateProvenanceResult = {
      verified: true,
      replayed: false,
      provenance: {
        provenance_id: 'modepack_candidate_provenance_123',
        ...
        signer_fingerprint: modePackSelectedCandidateFetchTarget.expected_signer_fingerprint,
        statement_sha256: modePackSelectedCandidateFetchTarget.expected_provenance_statement_sha256,
        signature_sha256: `sha256:${'9'.repeat(64)}`,
        verified_at: '2026-08-14T05:08:33Z',
        provenance_event_id: 'event_candidate_provenance_123',
      },
      next_action: 'approve_verified_modepack_candidate_explicitly',
    };
    const modePackProvenanceHeadlessResult = {
      ...headlessResult,
      selected_task_id: null,
      selected_run_id: null,
      task_run_result: null,
      ...
      next_route: {
        kind: 'approve_verified_modepack_candidate_explicitly',
        reason: 'Selected Mode Pack candidate provenance verified; approve explicitly.',
        next_action: 'approve_verified_modepack_candidate_explicitly',
      },
      next_action: 'approve_verified_modepack_candidate_explicitly',
    };
    expect(isHeadlessContinueOnceResult(modePackProvenanceHeadlessResult)).toBe(true);
    expect(isHeadlessContinueOnceResult({
      ...modePackProvenanceHeadlessResult,
      modepack_verify_candidate_provenance_result: { ...selectedCandidateProvenanceResult, raw_provenance_statement_json: '{}' },
    })).toBe(false);
    expect(isHeadlessContinueOnceResult({
      ...modePackProvenanceHeadlessResult,
      modepack_fetch_candidate_result: selectedCandidateFetchResult,
    ...
    expect(isHeadlessRunAdvanceParams({ ...headlessRunAdvanceParams, modepack_selected_candidate_provenance_verification_target: { ...modePackSelectedCandidateProvenanceVerificationTarget, raw_provenance_statement_json: '{}' } })).toBe(false);
    expect(isHeadlessRunAdvanceParams({ ...headlessRunAdvanceParams, modepack_selected_candidate_approval_target: { ...modePackSelectedCandidateApprovalTarget, raw_provenance_statement_json: '{}' } })).toBe(false);
    expect(isHeadlessRunAdvanceParams({ ...headlessRunAdvanceParams, modepack_selected_approved_candidate_replacement_target: { ...modePackSelectedApprovedCandidateReplacementTarget, raw_modepack_json: '{}' } })).toBe(false);
    ...
    const selectedProductGapClosureRequest = {
      authorize_selected_product_gap_closure: true,
      closure_id: 'm63.selected-gap-closure',
      source_decision_id: 'm51-product-decision',
      expected_source_decision_fingerprint: `sha256:${'9'.repeat(64)}`,
      expected_product_evidence_fingerprint: `sha256:${'a'.repeat(64)}`,
      expected_selected_remaining_gap_fingerprint: selectedRemainingGap.selection_fingerprint,
      expected_product_objective_fingerprint: `sha256:${'5'.repeat(64)}`,
      expected_accepted_completion_fingerprint: `sha256:${'d'.repeat(64)}`,
      expected_terminal_completion_fingerprint: terminalCompletionEvidence.completion_result_fingerprint,
      expected_completion_closure_fingerprint: `sha256:${'b'.repeat(64)}`,
    };
    const selectedProductGapClosureEvidence = {
      closure_id: selectedProductGapClosureRequest.closure_id,
      source_decision_id: selectedProductGapClosureRequest.source_decision_id,
      task_id: 'task_1',
      run_id: 'run_1',
      acceptance_id: 'm51-accepted-route',
      source_decision_fingerprint: selectedProductGapClosureRequest.expected_source_decision_fingerprint,
      product_evidence_fingerprint: selectedProductGapClosureRequest.expected_product_evidence_fingerprint,
      product_objective_fingerprint: selectedProductGapClosureRequest.expected_product_objective_fingerprint,
      selected_remaining_gap: selectedRemainingGap,
      accepted_completion_fingerprint: selectedProductGapClosureRequest.expected_accepted_completion_fingerprint,
      terminal_completion_fingerprint: selectedProductGapClosureRequest.expected_terminal_completion_fingerprint,
      completion_closure_fingerprint: selectedProductGapClosureRequest.expected_completion_closure_fingerprint,
      closure_evidence_fingerprint: `sha256:${'4'.repeat(64)}`,
      status: 'closed',
      next_action: 'derive_product_evidence_matrix_with_closed_gap',
      replayed: false,
    };
    ...
    const headlessRunDriveResult = {
      ...
      product_evidence_matrix: headlessRunProductEvidenceMatrix,
      selected_product_gap_closure: selectedProductGapClosureEvidence,
      product_completion_decision: {
        decision_id: 'm51-product-decision',
        task_id: 'task_1',
        run_id: 'run_1',
    ...
    const headlessRunJourneyClosure = {
      journey_id: 'm50.journey.1',
      authorize_journey_closure: true,
      expected_journey_fingerprint: `sha256:${'3'.repeat(64)}`,
      source_replacement_drive_id: 'm50.route.replacement',
      expected_replacement_resume_fingerprint: `sha256:${'6'.repeat(64)}`,
    };
    const headlessRunJourneyExecutionAdmission = {
      journey_id: 'm50.journey.1',
    ...
    const headlessRunJourneyExecution = {
      journey_id: 'm50.journey.1',
      authorize_journey_execution: true,
      expected_journey_fingerprint: `sha256:${'3'.repeat(64)}`,
      expected_execution_checkpoint_fingerprint: `sha256:${'a'.repeat(64)}`,
    };
    const headlessRunJourneyMetadata = {
      ...
      start_progress_fingerprint: `sha256:${'1'.repeat(64)}`,
      start_aggregate_sequence: 0,
      post_progress_fingerprint: `sha256:${'2'.repeat(64)}`,
      post_aggregate_sequence: 1,
      closure_status: 'budget_exhausted',
      next_action: 'inspect_progress_overview',
      replayed: false,
      journey_fingerprint: `sha256:${'3'.repeat(64)}`,
    };
    ...
    const headlessRunJourneyObjectiveContextMetadata = {
      ...
      objective_context_fingerprint: `sha256:${'9'.repeat(64)}`,
      selected_context_fingerprint: `sha256:${'0'.repeat(64)}`,
      prompt_context_id: 'prompt_context_1',
      ...
      read_path_fingerprint: `sha256:${'7'.repeat(64)}`,
      file_kind: 'Rust',
      bytes_read: selectedIndexContext.bytes_read,
      content_char_count: selectedIndexContext.content.length,
      content_sha256: selectedIndexContext.content_sha256,
      next_action: 'run_admitted_coding_task',
    };
    ...
    const headlessRunProvenanceJourneyRouteResumeMetadata = {
      ...headlessRunJourneyRouteResumeMetadata,
      route_kind: 'verify_selected_mode_pack_candidate_provenance_explicitly',
      derived_target_class: 'modepack_selected_candidate_provenance_verification_target',
      next_action: 'approve_verified_modepack_candidate_explicitly',
    };
    const headlessRunApprovalJourneyRouteResumeMetadata = {
      ...headlessRunJourneyRouteResumeMetadata,
      route_kind: 'approve_verified_mode_pack_candidate_explicitly',
    ...
    const headlessRunJourneyClosureMetadata = {
      journey_id: 'm50.journey.1',
      task_id: 'task_1',
      run_id: 'run_1',
      session_id: 'm50.journey',
      ...
      source_replacement_drive_id: 'm50.route.replacement',
      source_replacement_resume_fingerprint: `sha256:${'6'.repeat(64)}`,
      replacement_route_kind: 'replace_active_with_approved_mode_pack_candidate_explicitly',
      replacement_continuation_id: 'run.m50.journey.5',
      replacement_checkpoint_fingerprint: `sha256:${'7'.repeat(64)}`,
      active_modepack_activation_fingerprint: `sha256:${'8'.repeat(64)}`,
      closure_fingerprint: `sha256:${'b'.repeat(64)}`,
      finalization_fingerprint: `sha256:${'e'.repeat(64)}`,
      terminal_completion_fingerprint: terminalCompletionEvidence.completion_result_fingerprint,
      progress_fingerprint: `sha256:${'c'.repeat(64)}`,
      aggregate_sequence: taskListProgressOverview.aggregate_sequence + 2,
      next_action: 'complete_headless_journey',
      replayed: false,
      journey_closure_fingerprint: `sha256:${'9'.repeat(64)}`,
    };
    const headlessRunJourneyExecutionMetadata = {
      journey_id: 'm50.journey.1',
      task_id: 'task_1',
      run_id: 'run_1',
      session_id: 'm50.journey',
      drive_id: 'm50.journey.execute',
      journey_fingerprint: `sha256:${'3'.repeat(64)}`,
      completed_boundaries: [
        {
          boundary: 'admit_journey',
          drive_id: 'm50.journey.execute.admit',
          session_sequence: 1,
          drive_fingerprint: `sha256:${'1'.repeat(64)}`,
          replayed: false,
        },
        {
          boundary: 'fetch_selected_candidate',
          drive_id: 'm50.journey.execute.fetch',
          route_kind: 'fetch_selected_mode_pack_candidate_explicitly',
          session_sequence: 2,
          drive_fingerprint: `sha256:${'2'.repeat(64)}`,
          resume_fingerprint: `sha256:${'6'.repeat(64)}`,
          replayed: false,
        },
        {
          boundary: 'close_journey',
          drive_id: 'm50.journey.execute.closure',
          session_sequence: 5,
          drive_fingerprint: `sha256:${'5'.repeat(64)}`,
          journey_closure_fingerprint: `sha256:${'9'.repeat(64)}`,
          replayed: false,
        },
      ],
      ...
      next_action: 'complete_headless_journey',
      replayed: false,
      execution_checkpoint_fingerprint: `sha256:${'a'.repeat(64)}`,
    };
    ...
    expect(isHeadlessRunDriveParams({
      ...
      context_budget: contextBudget,
      journey_admission: {
        ...headlessRunJourneyAdmission,
        journey_id: 'm56.journey.1',
        objective_context: headlessRunJourneyObjectiveContext,
      },
    })).toBe(true);
    expect(isHeadlessRunDriveParams({
      authorize: true,
    ...
    expect(isHeadlessRunDriveParams({
      ...
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_route_resume: headlessRunProvenanceJourneyRouteResume,
    })).toBe(true);
    expect(isHeadlessRunDriveParams({
      authorize: true,
      session_id: 'm50.journey',
      drive_id: 'm50.route.approval',
    ...
    expect(isHeadlessRunDriveParams({
      ...
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_execution: headlessRunJourneyExecutionAdmission,
    })).toBe(true);
    expect(isHeadlessRunDriveParams({
      authorize: true,
      session_id: 'm50.journey',
      ...
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_execution: headlessRunJourneyExecution,
    })).toBe(true);
    expect(isHeadlessRunDriveParams({
      authorize: true,
      ...
      max_steps_per_advance: 1,
      expected_completion_closure_fingerprint: `sha256:${'b'.repeat(64)}`,
      product_evidence_derivation: headlessRunProductEvidenceDerivation,
      product_completion_decision: {
        ...headlessRunDriveParams.product_completion_decision,
        expected_product_evidence_fingerprint: `sha256:${'8'.repeat(64)}`,
        derived_product_evidence_matrix_fingerprint: `sha256:${'8'.repeat(64)}`,
      },
    })).toBe(true);
    expect(isHeadlessRunDriveParams({
      authorize: true,
    ...
    expect(isHeadlessRunDriveParams({
      ...
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_execution: { ...headlessRunJourneyExecutionAdmission, task_start: null },
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      authorize: true,
      session_id: 'm50.journey',
      ...
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_execution: { ...headlessRunJourneyExecutionAdmission, expected_journey_fingerprint: `sha256:${'3'.repeat(64)}` },
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      ...headlessRunDriveParams,
      max_advances: 2,
    ...
    expect(isHeadlessRunDriveParams({
      ...
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_closure: { ...headlessRunJourneyClosure, authorize_journey_closure: false },
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      ...headlessRunDriveParams,
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_execution: { ...headlessRunJourneyExecution, authorize_journey_execution: false },
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      ...headlessRunDriveParams,
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_execution: { ...headlessRunJourneyExecution, expected_journey_fingerprint: 'not-a-fingerprint' },
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      ...headlessRunDriveParams,
      max_advances: 1,
      max_steps_per_advance: 1,
      journey_execution: { ...headlessRunJourneyExecution, expected_execution_checkpoint_fingerprint: 'not-a-fingerprint' },
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      ...headlessRunDriveParams,
      max_advances: 2,
    ...
    expect(isHeadlessRunDriveParams({
      ...
      max_advances: 1,
      max_steps_per_advance: 1,
      authorize_completion_finalization: false,
      journey_closure: headlessRunJourneyClosure,
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      authorize: true,
    ...
    expect(isHeadlessRunDriveParams({
      ...
      expected_start_session_sequence: 0,
      max_advances: 1,
      journey_admission: { ...headlessRunJourneyAdmission, authorize_journey_start: false },
    })).toBe(false);
    expect(isHeadlessRunDriveParams({
      authorize: true,
      session_id: 'm50.journey',
    ...
    expect(isHeadlessRunDriveParams({
      ...
      journey_admission: {
        ...
        journey_id: 'm56.journey.1',
        objective_context: {
          ...headlessRunJourneyObjectiveContext,
          objective_fingerprint: 'not-a-fingerprint',
        },
      },
    })).toBe(false);
    ...
    expect(isHeadlessRunDriveParams({ ...headlessRunDriveParams, selected_product_gap_closure: selectedProductGapClosureRequest })).toBe(true);
    expect(isHeadlessRunDriveParams({ ...headlessRunDriveParams, selected_product_gap_closure: { ...selectedProductGapClosureRequest, authorize_selected_product_gap_closure: false } })).toBe(false);
    expect(isHeadlessRunDriveParams({ ...headlessRunDriveParams, selected_product_gap_closure: { ...selectedProductGapClosureRequest, expected_selected_remaining_gap_fingerprint: 'not-a-fingerprint' } })).toBe(false);
    expect(isHeadlessRunDriveParams({ ...headlessRunDriveParams, selected_product_gap_closure: { ...selectedProductGapClosureRequest, raw_product_gap: 'secret' } })).toBe(false);
    ...
    expect(isHeadlessRunDriveParams({ ...headlessRunDriveParams, modepack_selected_candidate_provenance_verification_target: { ...modePackSelectedCandidateProvenanceVerificationTarget, raw_provenance_statement_json: '{}' } })).toBe(false);
    ...
    expect(isHeadlessRunDriveResult({ ...headlessRunDriveResult, journey_closure: { ...headlessRunJourneyClosureMetadata, replacement_route_kind: 'approve_verified_mode_pack_candidate_explicitly' } })).toBe(false);
    expect(isHeadlessRunDriveResult({ ...headlessRunDriveResult, journey_closure: { ...headlessRunJourneyClosureMetadata, replacement_checkpoint_fingerprint: 'not-a-fingerprint' } })).toBe(false);
    expect(isHeadlessRunDriveResult({ ...headlessRunDriveResult, journey_closure: { ...headlessRunJourneyClosureMetadata, raw_modepack_json: '{}' } })).toBe(false);
    expect(isHeadlessRunDriveResult({ ...headlessRunDriveResult, journey_execution: { ...headlessRunJourneyExecutionMetadata, execution_checkpoint_fingerprint: 'not-a-fingerprint' } })).toBe(false);
    expect(isHeadlessRunDriveResult({ ...headlessRunDriveResult, journey_execution: { ...headlessRunJourneyExecutionMetadata, completed_boundaries: [{ ...headlessRunJourneyExecutionMetadata.completed_boundaries[0], boundary: 'unknown_boundary' }] } })).toBe(false);
    expect(isHeadlessRunDriveResult({ ...headlessRunDriveResult, journey_execution: { ...headlessRunJourneyExecutionMetadata, raw_prompt: 'secret' } })).toBe(false);
...
describe('RuntimeClient', () => {
  ...
  it('creates a modepack.approveCandidate request', async () => {
    const result = {
      approved: true,
      replayed: false,
      approval: {
  ...
  it('creates a modepack.replaceActive request for an approved candidate', async () => {
    ...
    const result = {
      ...
      update_admission: {
        ...
        source_url_host: approval.source_url_host,
    ...
    await expect(client.replaceActiveModePack(true, previous.activation_fingerprint, replacement.activation_fingerprint, {
      ...
      sourceUrlHost: approval.source_url_host,
      sourceUrlFingerprint: approval.source_url_fingerprint,
      dnsResolutionFingerprint: approval.dns_binding.resolution_fingerprint,
      pinnedAddressFingerprint: approval.dns_binding.pinned_address_fingerprint,
    ...
    expect(transport.requests[0].params).toMatchObject({
      ...
      expected_approved_candidate_id: approval.candidate_id,
      expected_approved_candidate_source_url_host: approval.source_url_host,
      expected_approved_candidate_source_url_fingerprint: approval.source_url_fingerprint,
      expected_approved_candidate_dns_resolution_fingerprint: approval.dns_binding.resolution_fingerprint,
      expected_approved_candidate_pinned_address_fingerprint: approval.dns_binding.pinned_address_fingerprint,
      expected_approved_candidate_approval_event_id: approval.approval_event_id,
      update_admission: {
  ...
  it('creates a modepack.rollbackActive request', async () => {
    ...
    await expect(client.rollbackActiveModePack(true, current.activation_fingerprint, restored.activation_fingerprint)).resolves.toEqual(result);
    expect(transport.requests).toEqual([{
      jsonrpc: '2.0',
      id: 1,
      method: 'modepack.rollbackActive',
      params: {
        authorize_rollback: true,
        expected_current_activation_fingerprint: current.activation_fingerprint,
        expected_rollback_activation_fingerprint: restored.activation_fingerprint,
      },
    }]);
  });
