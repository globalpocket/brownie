...
describe('protocol validation', () => {
  ...
  it('accepts proposal.list results and rejects raw content fields', () => {
    ...
    const auditEntry = { proposal_id: 'proposal_1', event_name: 'apply_dry_run_checked', ledger_kind: 'WorkspacePatchApplyDryRunChecked', occurred_at: '2026-07-01T00:00:00Z', metadata: { proposal_id: 'proposal_1', dry_run_id: 'apply_dry_run_1', apply_executed: false, workspace_files_changed: false } };
    ...
    expect(isProposalAuditTrailResult({ proposal: result.proposals[0], audit_trail: { ...auditTrail, events: [{ ...auditEntry, metadata: { ...auditEntry.metadata, file_content: 'secret' } }] } })).toBe(false);
    expect(isProposalAuditTrailResult({ proposal: result.proposals[0], audit_trail: { ...auditTrail, latest_event: { ...auditEntry, patch: 'raw' } } })).toBe(false);
...
describe('RuntimeClient', () => {
  ...
  it('creates a proposal.auditTrail request', async () => {
    const proposal = { proposal_id: 'proposal_1', path: 'README.md', operation: 'replace_file', content_preview: 'new', content_chars: 3, truncated: false, validation_status: 'Valid', validation_reason: null, diff_preview: '--- a/README.md', diff_truncated: false, diff_redacted: false, approval_status: 'Approved', approval_reason: 'ok', approved_at: '2026-06-30T00:00:00Z', rejected_at: null, approval_reason_redacted: false };
    const entry = { proposal_id: 'proposal_1', event_name: 'proposal_approved', ledger_kind: 'WorkspacePatchApproved', occurred_at: '2026-07-01T00:00:00Z', metadata: { proposal_id: 'proposal_1', approval_status: 'Approved' } };
