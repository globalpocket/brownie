...
describe('protocol validation', () => {
  ...
  it('accepts proposal.list results and rejects raw content fields', () => {
    ...
    const auditEntry = { proposal_id: 'proposal_1', event_id: 'event_1', event_kind: 'WorkspacePatchApplyDryRunChecked', audit_event: 'apply_dry_run_checked', timestamp: '2026-07-01T00:00:00Z', summary: 'Apply dry-run checked without applying a patch or changing workspace files.', metadata: { dry_run_id: 'apply_dry_run_1', no_patch_applied: true, apply_executed: false, workspace_files_changed: false } };
    ...
    expect(isProposalAuditTrailResult({ proposal: result.proposals[0], audit_trail: { ...auditTrail, latest_event: { ...auditEntry, metadata: { patch: 'raw' } } } })).toBe(false);
    expect(isProposalAuditTrailResult({ proposal: result.proposals[0], audit_trail: { ...auditTrail, events: [{ ...auditEntry, metadata: { nested: { file_content: 'secret' } } }] } })).toBe(false);
    expect(isProposalAuditTrailResult({ proposal: result.proposals[0], audit_trail: { ...auditTrail, events: [{ ...auditEntry, diff: 'raw' }] } })).toBe(false);
...
describe('RuntimeClient', () => {
  ...
  it('creates a proposal.auditTrail request', async () => {
    const proposal = { proposal_id: 'proposal_1', path: 'README.md', operation: 'replace_file', content_preview: 'new', content_chars: 3, truncated: false, validation_status: 'Valid', validation_reason: null, diff_preview: '--- a/README.md', diff_truncated: false, diff_redacted: false, approval_status: 'Approved', approval_reason: 'ok', approved_at: '2026-06-30T00:00:00Z', rejected_at: null, approval_reason_redacted: false };
    const auditEntry = { proposal_id: 'proposal_1', event_id: 'event_1', event_kind: 'WorkspacePatchProposed', audit_event: 'proposal_created', timestamp: '2026-07-01T00:00:00Z', summary: 'Proposal created for workspace patch.', metadata: { path: 'README.md', operation: 'replace_file', validation_status: 'Valid' } };
    const audit_trail = { proposal_id: 'proposal_1', event_count: 1, latest_event: auditEntry, events: [auditEntry], generated_at: '2026-07-01T00:01:00Z' };
    const transport = new FakeTransport({ jsonrpc: '2.0', id: 1, result: { proposal, audit_trail } });
    await expect(new RuntimeClient(transport).auditTrailProposal('run_1', 'proposal_1')).resolves.toEqual({ proposal, audit_trail });
